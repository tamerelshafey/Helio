...
interface CustomDecorationRequestModalProps {
    onClose: () => void;
    categoryName: string;
    language: Language;
}

const CustomDecorationRequestModal: React.FC<CustomDecorationRequestModalProps> = ({ onClose, categoryName, language }) => {
    const t = translations[language].customDecorationRequestModal;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const finalNotes = `${formData.customerNotes}${referenceImage ? `\n\n(Reference Image Uploaded: ${referenceImage.name})` : ''}`;
        const serviceTitle = `${t.serviceTitle}: ${categoryName}`;

        // In a real app, you'd find a specific partner or have a general inbox
        // For this simulation, we'll assign it to a relevant partner.
        const partnerId = 'artistic-touch-decor'; 
            ...
            >
                <div className="p-8">
                    ...
                    <div className="text-center mb-6">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">{`${t.categoryLabel}: ${categoryName}`}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <FormField label={t.ideaDescription} id="customerNotes">
                            <textarea id="customerNotes" name="customerNotes" rows={4} value={formData.customerNotes} onChange={handleChange} placeholder={t.ideaPlaceholder} className={inputClasses}></textarea>
                        </FormField>
                        <FormField label={t.referenceImage} id="referenceImage">
