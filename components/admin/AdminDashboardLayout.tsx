...
const AdminDashboardLayout: React.FC = () => {
    ...
    const dashboardTitle = t.partnerInfo[user.id]?.name || user.name;
    ...
    const SidebarContent = () => (
      <div className="flex flex-col h-full">
            ...
            <nav className="flex-grow overflow-y-auto overflow-x-hidden -mr-2 pr-2 dashboard-sidebar">
                {linkGroups.map((group, index) => (
                    <ul key={index} className="space-y-1 border-t border-gray-200 dark:border-gray-700 pt-4 mt-4 first:border-t-0 first:mt-0 first:pt-0">
                         <h3 className={`px-4 mb-2 text-xs font-semibold text-gray-400 uppercase tracking-wider ${isSidebarCollapsed ? 'text-center' : ''}`}>{isSidebarCollapsed ? group.name.substring(0,1) : group.name}</h3>
                        {group.links.map(link => {
                            const Icon = link.icon;
                            const linkName = link.name(t_admin_dashboard);
                            return (
                            <li key={link.href}>
