...
const AdminContentLayout: React.FC = () => {
    ...
    const navLinks = [
        ...
        { key: 'services', href: 'services', icon: SparklesIcon, label: 'Services' },
        ...
        { key: 'quotes', href: 'quotes', icon: QuoteIcon, label: 'Quotes' },
        { key: 'footer', href: 'footer', icon: PhoneIcon, label: 'Footer' },
    ];
    
    const baseLinkClasses = "flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 group";
    const activeLinkClasses = "bg-amber-500 text-gray-900 shadow-md transform scale-105";
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full">
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <nav className="w-full lg:w-64 flex-shrink-0 bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden sticky top-24">
                    <div className="p-4 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                        <h3 className="font-bold text-gray-700 dark:text-gray-200 text-sm uppercase tracking-wider">Sections</h3>
                    </div>
                        ...
                        {navLinks.map(link => {
                            ...
                            return (
                                <li key={link.key}>
                                    ...
                                    >
                                        <Icon className={`w-5 h-5 ${language === 'ar' ? 'ml-2' : 'mr-2'}`} />
