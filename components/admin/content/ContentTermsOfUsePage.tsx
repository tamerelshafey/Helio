...
const ContentTermsOfUsePage: React.FC = () => {
    const { register, control } = useOutletContext<ContentFormContext>();
    
    // Arrays for sections
    const { fields: arFields, append: appendAr, remove: removeAr, swap: swapAr } = useFieldArray({ control, name: 'termsOfUse.ar.sections' });
    const { fields: enFields, append: appendEn, remove: removeEn, swap: swapEn } = useFieldArray({ control, name: 'termsOfUse.en.sections' });
        ...
        <div className="space-y-8 animate-fadeIn">
                ...
                <CardContent className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <Input {...register('termsOfUse.ar.title')} dir="rtl" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Last Updated Text</label>
                            <Input {...register('termsOfUse.ar.lastUpdated')} dir="rtl" />
                        </div>
                    </div>
                     <div className="space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1">Page Title</label>
                            <Input {...register('termsOfUse.en.title')} dir="ltr" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Last Updated Text</label>
                            <Input {...register('termsOfUse.en.lastUpdated')} dir="ltr" />
                        </div>
                    </div>
                </CardContent>
            </Card>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">Terms Sections</h3>
                        ...
                        <CardContent className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Input {...register(`termsOfUse.ar.sections.${index}.title`)} dir="rtl" className="font-bold" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium mb-1 text-gray-500">Content (Markdown supported)</label>
                                    <Textarea {...register(`termsOfUse.ar.sections.${index}.content`)} rows={6} dir="rtl" />
                                </div>
                            </div>
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-xs font-medium mb-1 text-gray-500">Title</label>
                                    <Input {...register(`termsOfUse.en.sections.${index}.title`)} dir="ltr" className="font-bold" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium mb-1 text-gray-500">Content (Markdown supported)</label>
                                    <Textarea {...register(`termsOfUse.en.sections.${index}.content`)} rows={6} dir="ltr" />
                                </div>
                            </div>
                        </CardContent>
                    </Card>
...
export default ContentTermsOfUsePage;
