...
import { Request, RequestType, Property } from '../../../types';
        ...
        mutationFn: async ({ status, req }: { status: 'approved' | 'rejected', req: Request }) => {
            if (status === 'approved') {
                if (req.type === RequestType.PARTNER_APPLICATION) {
                    ...
                    const pd = (req.payload as any).propertyDetails;
                    ...
                    const newProperty: Omit<Property, 'id' | 'partnerName' | 'partnerImageUrl'> = {
                        ...
                        imageUrl: (req.payload as any).images[0] || '',
                        gallery: (req.payload as any).images.slice(1),
