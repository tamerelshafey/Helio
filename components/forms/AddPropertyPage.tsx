                        ...
                        <div className="bg-white p-8 rounded-lg border border-gray-200 sticky top-28">
                            ...
                            <ul className="space-y-5">
                                <li className="flex items-start">
                                    ...
                                    <p className={`text-gray-600 ${language === 'ar' ? 'mr-4' : 'ml-4'}`}>{t_page.step1}</p>
                                </li>
                                 <li className="flex items-start">
                                    ...
                                    <p className={`text-gray-600 ${language === 'ar' ? 'mr-4' : 'ml-4'}`}>{t_page.step2}</p>
                                </li>
                                <li className="flex items-start">
                                    ...
                                    <p className={`text-gray-600 ${language === 'ar' ? 'mr-4' : 'ml-4'}`}>{t_page.step3}</p>
                                </li>
                                <li className="flex items-start">
                                    ...
                                    <p className={`text-gray-600 ${language === 'ar' ? 'mr-4' : 'ml-4'}`}>{t_page.step4}</p>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <div className="bg-white p-8 rounded-lg border border-gray-200">
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                ...
                                {cooperationType === 'paid_listing' && (
                                    <fieldset className="space-y-2 border-t border-gray-200 pt-4 animate-fadeIn">
                                ...
                                <fieldset className="space-y-4 border-t border-gray-200 pt-4">
                                    <legend className="text-lg font-semibold text-amber-500 mb-2">{t_page.propertyInfo}</legend>
                                     <div className="grid sm:grid-cols-2 gap-4">
                                        <FormField label={t_page.purpose} id="purpose" error={errors.purpose?.message}>
                                            <Select {...register("purpose", { required: t_page.errors.required })} className={!watch("purpose") ? 'text-gray-500' : ''} >
                                                <option value="" disabled>{t_page.selectPurpose}</option>
                                                {purposeOptions.map(opt => <option key={opt.en} value={opt.en} className="text-gray-900">{opt[language]}</option>)}
                                            </Select>
                                        </FormField>
                                        <FormField label={t_page.propertyType} id="propertyType" error={errors.propertyType?.message}>
                                             <Select {...register("propertyType", { required: t_page.errors.required })} className={!watch("propertyType") ? 'text-gray-500' : ''} disabled={isLoadingContext}>
                                                <option value="" disabled>{isLoadingContext ? 'Loading...' : t_page.selectType}</option>
                                                {(propertyTypes || []).map(opt => <option key={opt.id} value={opt.en} className="text-gray-900">{opt[language]}</option>)}
                                            </Select>
                                        </FormField>
                                     </div>
                                      <div className="grid sm:grid-cols-2 gap-4">
                                        {watchPropertyType !== 'Land' && (
                                            <FormField label={t_page.finishingStatus} id="finishingStatus" error={errors.finishingStatus?.message}>
                                                <Select {...register("finishingStatus", { required: watchPropertyType !== 'Land' ? t_page.errors.required : false })} className={!watch("finishingStatus") ? 'text-gray-500' : ''} disabled={isLoadingContext}>
                                                    <option value="" disabled>{isLoadingContext ? 'Loading...' : t_page.selectFinishing}</option>
                                                    {(finishingStatuses || []).map(opt => <option key={opt.id} value={opt.en} className="text-gray-900">{opt[language]}</option>)}
                                                </Select>
                                            </FormField>
                                        )}
                                    ...
                                    <fieldset className="space-y-4 border-t border-gray-200 pt-4">
                                        ...
                                        <button type="button" onClick={() => setIsLocationModalOpen(true)} className="w-full text-center font-medium text-sm text-amber-600 hover:underline p-2 rounded-md border border-amber-500 hover:bg-amber-50">
                                            Select on Map
                                        </button>
                                    </fieldset>
                                ...
                                <fieldset className="space-y-4 border-t border-gray-200 pt-4">
                                    ...
                                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                        <FormField label={t_page.inCompound} id="isInCompound">
                                            <RadioGroup className="flex gap-4 pt-2">
                                                <label className="flex items-center gap-2"><RadioGroupItem {...register("isInCompound")} value="yes" id="compound-yes" /> {t_page.yes}</label>
                                                <label className="flex items-center gap-2"><RadioGroupItem {...register("isInCompound")} value="no" id="compound-no" /> {t_page.no}</label>
                                            </RadioGroup>
                                        </FormField>
                                        <FormField label={t_page.deliveryDate} id="deliveryType">
                                             <RadioGroup className="flex gap-4 pt-2">
                                                <label className="flex items-center gap-2"><RadioGroupItem {...register("deliveryType")} value="immediate" id="delivery-immediate"/> {t_page.immediate}</label>
                                                <label className="flex items-center gap-2"><RadioGroupItem {...register("deliveryType")} value="future" id="delivery-future"/> {t_page.future}</label>
                                            </RadioGroup>
                                        </FormField>
                                        <FormField label={t_page.realEstateFinance} id="realEstateFinanceAvailable">
                                            <RadioGroup className="flex gap-4 pt-2">
                                                <label className="flex items-center gap-2"><RadioGroupItem {...register("realEstateFinanceAvailable")} value="yes" id="finance-yes" /> {t_page.yes}</label>
                                                <label className="flex items-center gap-2"><RadioGroupItem {...register("realEstateFinanceAvailable")} value="no" id="finance-no"/> {t_page.no}</label>
                                            </RadioGroup>
                                        </FormField>
                                    </div>
                                    {watchDeliveryType === 'future' && (
                                        <div className="grid sm:grid-cols-2 gap-4 p-4 border border-gray-200 rounded-md">
                                     ...
                                     <FormField label={t_page.installments} id="hasInstallments">
                                        <RadioGroup className="flex gap-4 pt-2">
                                            <label className="flex items-center gap-2"><RadioGroupItem {...register("hasInstallments")} value="yes" id="installments-yes" /> {t_page.yes}</label>
                                            <label className="flex items-center gap-2"><RadioGroupItem {...register("hasInstallments")} value="no" id="installments-no" /> {t_page.no}</label>
                                        </RadioGroup>
                                    </FormField>
                                     {watchHasInstallments === 'yes' && (
                                        <div className="grid sm:grid-cols-3 gap-4 p-4 border border-gray-200 rounded-md">
                                ...
                                <div className="space-y-4 border-t border-gray-200 pt-4">
                                     <div className="flex items-start">
                                        ...
                                        />
                                        <label htmlFor="isOwner" className={`text-sm text-gray-600 ${language === 'ar' ? 'mr-3' : 'ml-3'}`}>
                                            {t_page.confirmationLabel}
                                        </label>
                                    </div>
                                    {errors.isOwner && <p className="text-red-500 text-sm">{errors.isOwner.message}</p>}
                                    <button type="submit" disabled={mutation.isPending} className="w-full bg-amber-500 text-gray-900 font-bold px-6 py-3 rounded-lg hover:bg-amber-600 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
